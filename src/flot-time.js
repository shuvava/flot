...
function addProxyMethod(sourceObj, sourceMethod, targetObj, targetMethod) {
    sourceObj[sourceMethod] = (...args) => targetObj[targetMethod](...args);
}
...
function tickGenerator(opts, axis) {
    const ticks = [];
    ...
    let size;
    let unit;
    for (let i = 0; i < spec.length - 1; i += 1) {
    ...
    if (unit === 'year') {
        ...
        if (opts.minTickSize != null && opts.minTickSize[1] === 'year') {
            ...
            const magn = 10 ** Math.floor(Math.log(axis.delta / timeUnitSize.year) / Math.LN10);
...
function tickFormatter(opts, v, axis) {
...
function processOptions(plot) {
    const allAxes = plot.getAxes();
    forEach(Object.values(allAxes), (axis) => {
        const opts = axis.options;
        if (opts.mode === 'time') {
            axis.tickGenerator = _axis => tickGenerator(opts, _axis);
            axis.tickFormatter = (v, _axis) => tickFormatter(opts, v, _axis);
        }
    });
}
